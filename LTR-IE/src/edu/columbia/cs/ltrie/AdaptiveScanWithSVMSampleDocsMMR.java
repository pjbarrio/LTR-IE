...
public class AdaptiveScanWithSVMSampleDocsMMR {
...
//						continue;
	...
	private static Map<String, Float> getMaxSimilarityScores(
		...
		for (String doc : docsToSelect) {
			...
			List<edu.columbia.cs.utils.Pair<String, Float>> res = conn.getScoresOrdered(weights, s, setDocs,noDocs);
			
			if (res.isEmpty())
				ret.put(doc, 0.0f);
			else
				ret.put(doc, res.get(0).second());
		...
		Collections.sort(collection, new Comparator<String>() {
			...
			public int compare(String doc1, String doc2) {
				...
				if(score2==null){
					...
					if (scores2.get(doc2) > 0.0)
						System.err.println(scores.get(doc2) + " -2- " + scores2.get(doc2));
				...
				if(score1==null){
					...
					if (scores2.get(doc2) > 0.0)
						System.err.println(scores.get(doc1) + " -1- " + scores2.get(doc1));

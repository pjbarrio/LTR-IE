...
public class GenerateQueriesSVMDocBySentence {
	...
	public static void main(String[] args) throws Exception {
		...
		for(String relationship : relationships){
			...
			CompressedAdditiveFileSystemWrapping extractWrapperDocuments = new CompressedAdditiveFileSystemWrapping();
			for(String subPath : subPaths){
				if(relationship.equals("OrgAff") || relationship.equals("Outbreaks")){
					extractWrapper.addFiles(resultsPath + "/" + subPath + "_" + relationship + "-sentence.data");
					extractWrapperDocuments.addFiles(resultsPath + "/" + subPath + "_" + relationship + ".data");
				}else{
					extractWrapper.addFiles(resultsPath + "/" + subPath + "_" + extractor + "_" + relationship + "-sentence.data");
					extractWrapperDocuments.addFiles(resultsPath + "/" + subPath + "_" + extractor + "_" + relationship + ".data");
				}
			}
			...
			for(String doc : sample){
				List<Tuple> tuples = extractWrapperDocuments.getTuplesDocument(doc);
				if(tuples.size()!=0){
			...
			Map<String,Float> scoresCollection = conn.getScores(queryScores, new SimpleBooleanSimilarity(), knownDocumentsToProcess);
			...
			for(int i=0; i<collectionSize; i++){
				...
				if(num!=0){
					...
					for (Integer local_sent : local_sents) {
						...
						int tuplesN = extractWrapper.getNumTuplesDocument(local_sent_str);
				...
				|| (updateEveryHalf && (i==nextUpdate && nextUpdate!=0))){
					...
					Map<String,Float> newScoresCollection = conn.getScores(newQueryScores, new SimpleBooleanSimilarity(), knownDocumentsToProcess);
			...
			for(String doc : collection){
				if(!knownDocuments.contains(doc)){
					int num = extractWrapperDocuments.getNumTuplesDocument(doc);
					if(num!=0){
						...
						for (Integer local_sent : local_sents) {
							...
							int tuples = extractWrapper.getNumTuplesDocument(local_sent_str);
							
							if(tuples!=0){
								relevance.put(local_sent_str, tuples);
							}
			...
			SerializationHelper.write("resultsRank/search" + docLimit + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + split + "/baseline.data", new BaselineCurve(numDocs));
			SerializationHelper.write("resultsRank/search" + docLimit + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + split + "/perfect.data", new PerfectCurve(numDocs, relevance));
			...
			SerializationHelper.write("resultsRank/search" + docLimit + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + split + "/initialRankSVM.data", new SortedCurve(numDocs, initialRanking, relevance));
			SerializationHelper.write("resultsRank/search" + docLimit + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + split + "/adaptiveRankSVM.data", new SortedCurve(numDocs, adaptiveRanking, relevance));
			SerializationHelper.write("resultsRank/search" + docLimit + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + split + "/adaptiveRankSVMUpdates.updates", updates);
			SerializationHelper.write("resultsRank/search" + docLimit + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + split + "/adaptiveRankSVMTimes.times", measurer.getCheckPoints());
			SerializationHelper.write("resultsRank/search" + docLimit + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + split + "/adaptiveRankSVMtuples.tuples", allTuples);
		}
	}

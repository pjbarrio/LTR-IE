...
public class AdaptiveScanWithSVMBagg {
	public static void main(String[] args) throws Exception {
		...
		String extractor = args[6]; //"Pablo-Dependency-Graph","Pablo-N-Grams","Pablo-Shortest-Path","Pablo-Sub-sequences"
		...
		for(String relationship : relationships){
			...
			if (extractor.equals("OpenCalais")){
				...
				String mfolderDesign = "%0" + String.valueOf(splits).length() + "d";
				for(int i=1; i<=splits; i++){
					String spath =String.format(mfolderDesign, i);
					
					String fName = resultsPath + "/" + spath + task + "_" + extractor + "_" + relationship + "-sentence.data";

					if (new File(fName).exists()){
						extractWrapper.addFiles(fName);
			...
			Combiner combi =  new SumCombiner();//comb.equals("Max")? new MaxCombiner() : new SumCombiner();

			ActiveLearningOnlineModel model = new ActiveLearningOnlineModel(docs,relevantDocs,coordinator,5,combi,"L1",0.5,true, false);
			...
			Map<String,Double> scoresCollection = model.getScores(collection,conn,true);
			...
			for(int i=0; i<collectionSize; i++){
				...
				if((updateDecision != null && updateDecision.doUpdate(currentBatchDocs, currentBatchRelDocs)) || (updatePrediction != null && updatePrediction.predictUpdate(rankedCollection,i))){
					...
					Map<String,Double> newScoresCollection = model.getScores(collection,conn,true);
			...
			SerializationHelper.write("resultsRank/full" + suffix + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + extractor + "/" + split + "/initialBagg.data", new SortedCurve(numDocs, initialRanking, relevance));
			SerializationHelper.write("resultsRank/full" + suffix + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + extractor + "/" + split+ "/adaptiveBagg.data", new SortedCurve(numDocs, adaptiveRanking, relevance));
			SerializationHelper.write("resultsRank/full" + suffix + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + extractor + "/" + split + "/adaptiveBaggUpdates.updates", updates);
			SerializationHelper.write("resultsRank/full" + suffix + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + extractor + "/" + split + "/adaptiveBaggUpdates.updates.statistics", updateStatistics);
			SerializationHelper.write("resultsRank/full" + suffix + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + extractor + "/" + split + "/adaptiveBaggTimes.times", measurer.getCheckPoints());
			SerializationHelper.write("resultsRank/full" + suffix + "/" + sampling + "/"+ updateMethod + "/" + relationship + "/" + extractor + "/" + split + "/adaptiveBaggtuples.tuples", allTuples);
			if(updateDecision!=null){

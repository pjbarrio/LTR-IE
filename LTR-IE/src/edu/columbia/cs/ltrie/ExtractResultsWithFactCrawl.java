...
public class ExtractResultsWithFactCrawl {
	public static void main(String[] args) throws IOException, ClassNotFoundException, InvalidVectorIndexException, RowsExceededException, WriteException, ParseException {
		...
		for(String relationship : relationships){
			...
			ExtractionStrategy strategyf = new StaticExtractionStrategy(conn,graph,scores,measurer); // Regular FactCrawl
			List<String> sortedDocumentsf = factCrawl.sortDocuments(graph.getAllDocs(),strategyf);
			
			System.out.println("I will process " + sortedDocumentsf.size() + " documents.");

			/*System.out.println("Re-rank Adaptive");
			Set<Integer> allDocs = graph.getAllDocs();
			...
			SerializationHelper.write("resultsRank/" + scenario +"/" + sampling + "/factcrawl/" + relationship + "/" + split + "/initial.data", new SortedCurve(numDocs, sortedDocumentsf, relevance));
			SerializationHelper.write("resultsRank/" + scenario +"/" + sampling + "/factcrawl/" + relationship + "/" + split + "/initialTimes.Times", measurer.getCheckPoints());

...
public class ElasticNetLinearPegasosConstantLearningRateEngine<E> implements LinearOnlineEngine<E> {
	...
	private ElasticNetLinearPegasosConstantLearningRateEngine(){
		
	}

	public ElasticNetLinearPegasosConstantLearningRateEngine(double lambda, double importanceL2, double hingeParameter, boolean randomRestart) {
		this(lambda,importanceL2,hingeParameter,true,randomRestart);
	}

	public ElasticNetLinearPegasosConstantLearningRateEngine(double lambda, double importanceL2, double hingeParameter, boolean useB,
	...
	public boolean updateModel(DataObject<Map<E, Double>> inputVector, boolean desiredOutput){
		...
		u+=((1.0-importanceL2)*lambda*learning);
		...
		double multConstant=1.0/(Math.sqrt(normSq*lambda));
		if(normSq==0){
			multConstant=0;
		}
	...
	public LinearOnlineEngine<E> copy() {
		ElasticNetLinearPegasosConstantLearningRateEngine<E> newEngine = new ElasticNetLinearPegasosConstantLearningRateEngine<E>();
		...
		newEngine.randomRestart=randomRestart;

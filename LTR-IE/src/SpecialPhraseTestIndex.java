...
public class SpecialPhraseTestIndex {
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		query1.add(new Term("BODY", "new"));
		query1.add(new Term("BODY", "york"));
		System.out.println(query1);
		Map<Query,Double> weights = new HashMap<Query, Double>();
		weights.put(query1, 1.0);
		
		int i=0;
		for(Entry<String,Float> entry : conn.getScores(weights, new SimpleBooleanSimilarity(),documents).entrySet()){
